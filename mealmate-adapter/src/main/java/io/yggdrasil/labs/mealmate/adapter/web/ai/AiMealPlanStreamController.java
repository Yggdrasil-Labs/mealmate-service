...
/**
 * AI 周餐计划流式生成 Controller。
 *
 * <p>通过 SSE (Server-Sent Events) 流式输出 AI 配餐计划生成结果。 使用独立线程池 aiStreamExecutor 异步执行，避免阻塞 Servlet 请求线程。
 * 客户端断连、超时或异常时通过 cancelled 标志通知上游停止生产。
 */
@Validated
@RestController
@Slf4j
@RequestMapping("/api/ai/meal-plans")
@Tag(name = "AI MealPlan Stream", description = "SSE streaming for AI meal plan generation.")
public class AiMealPlanStreamController {

    private final AiMealPlanAppService aiMealPlanAppService;
    private final TaskExecutor aiStreamExecutor;

    public AiMealPlanStreamController(
            AiMealPlanAppService aiMealPlanAppService,
            @Qualifier("aiStreamExecutor") TaskExecutor aiStreamExecutor) {
        this.aiMealPlanAppService = aiMealPlanAppService;
     ...
     *   <li>chunk: LLM 增量文本片段
     *   <li>done: 流结束标志 "[DONE]"
     *   <li>result: 完整配餐结果 JSON
     *   <li>error: 错误信息 JSON（含 code 和 message）
     * </ul>
     */
    @PostMapping(value = "/generate/stream", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
    @Operation(summary = "AI 流式生成周计划", description = "SSE 流式输出 AI 生成的一周三餐计划，逐 chunk 返回")
    public SseEmitter generateStream(@RequestBody @Valid AiMealPlanGenerateCmd cmd) {
        ...
        aiStreamExecutor.execute(
                () -> {
                    try {
                        aiMealPlanAppService.generateStream(
                                cmd,
                                chunk -> {
                                    if (!cancelled.get()) {
                        ...
                        log.error("[AI Stream] Unexpected error in meal plan stream", e);
