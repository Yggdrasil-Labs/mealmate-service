...
> 本文件是智能体了解代码库"已知问题"的入口。智能体在修改某个领域前，应先查看该领域是否有已知债务。智能体在完成任务后发现新的债务，应记录到此文件。本文件属于"长期维护清单"，永不归档；由 doc-gardening agent 持续维护，详见 `docs/guides/PLANS.md` 的"持续维护节奏"。

| 编号 | 领域 | 问题描述 | 优先级 | 记录日期 | Owner | 关联计划 |
|------|------|----------|--------|----------|-------|----------|
| TD1 | 文档导航 | `docs/business/` 与 `docs/plans/` 已从主导航中移除并收口到新体系，仍需持续保证后续新增内容不回流到旧目录模式 | 中 | 2026-04-14 | doc-gardening | — |
| TD2 | 文档结构 | `docs/exec-plans/` 需要迁移到 `docs/active/` 和 `docs/archive/` 结构 | 高 | 2026-05-08 | doc-gardening | — |
