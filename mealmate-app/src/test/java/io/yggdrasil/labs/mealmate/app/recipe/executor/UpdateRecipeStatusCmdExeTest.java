...
class UpdateRecipeStatusCmdExeTest {
    ...
    @Test
    void shouldRejectUpdateStatusWhenRecipeIsSystem() {
        UpdateRecipeStatusCmd cmd = new UpdateRecipeStatusCmd(1L, RecipeStatus.INACTIVE);

        when(recipeRepository.findById(1L)).thenReturn(Optional.of(systemRecipe(1L)));

        UpdateRecipeStatusCmdExe cmdExe =
                new UpdateRecipeStatusCmdExe(recipeRepository, recipeDomainService);
